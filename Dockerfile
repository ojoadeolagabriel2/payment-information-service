...
RUN jlink \
    --verbose \
    --add-modules jdk.crypto.ec,jdk.unsupported,java.base,java.sql,java.naming,java.desktop,java.management,java.security.jgss,java.instrument,jdk.jdwp.agent \
    --compress 2 \
    --strip-debug \
    --no-header-files \
    --no-man-pages \
...
COPY "/target/pokemon-information-service.jar" "/app.jar"

EXPOSE 50000
CMD [ "-Djdk.tls.client.protocols=TLSv1.2", "-jar", "/app.jar" ]
ENTRYPOINT [ "java" ]